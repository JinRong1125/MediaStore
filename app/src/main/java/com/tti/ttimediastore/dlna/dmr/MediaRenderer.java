...
public class MediaRenderer {
	...
	public MediaRenderer(final Context context) {
		DeviceType type = new UDADeviceType(DMRUnity.DMR, DMRUnity.VERSION);
		DeviceDetails details = new DeviceDetails(android.os.Build.MODEL,
				new ManufacturerDetails(DMRUnity.MANUFACTURER,
						DMRUnity.MANUFACTURER_URL),
				new ModelDetails(DMRUnity.DMR_MODEL_NAME,
						DMRUnity.DMR_MODEL_DESCRIPTION, DMRUnity.MODEL_NUMBER));
		...
		try {
			...
			mLocalDevice = new LocalDevice(new DeviceIdentity(Utils.Preferences.getDmrUDN()),
					type, details,
	...
	private Icon createDefaultDeviceIcon(Context context) {
		BitmapDrawable bitDw = ((BitmapDrawable) TTIMediaStore.getInstance().getResources()
				.getDrawable(R.drawable.tti_logo));
